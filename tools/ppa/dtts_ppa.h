...
#include <map>
#include <set>
typedef std::map<node_t,bool> mark_t;
...
public:
    ...
    void add_new_edge(const node_t& pnode1, const node_t& pnode2)
    ...
    */
    ...
    bool ridge_type;
    void shaunPPA(Terrain& src, bool ridges=true, int srate = 2, int plength = 7, int portion = 20);
    ...
    void extractFromSet(Terrain& src, std::set<node_t> nodes);

    void runPPA(Terrain& src, bool is_ridge=true, int plength=7, int radius = 25,int groupmin=10, int branchmin=40, int samplerate=5, float ethres=0.0)
    {
        ...
        if (is_ridge)
            cerr<< " Ridge extraction from a terrain of size (" <<src.width()<<","<<src.height()<<")\n";
        else
            cerr<< " Valley extraction from a terrain of size (" <<src.width()<<","<<src.height()<<")\n";
    ...
    void runPPA_bis(Terrain& src, bool is_ridge=true, int plength=7, int radius = 25,int groupmin=10, int branchmin=40, int samplerate=5, float ethres=0.0)
    {
        clock_t tstart = clock();
    ...
    void loadFile(Terrain& src, const char* fname, bool is_ridge=true){
    ...
    Terrain renderFeatures(float color=100.);
    ...
    */
    ...
    Terrain renderFollowers(Terrain patch, int cx, int cy, int bsize, float color);
